...
(function () {
	...
	function reporter(errors) {
		...
		[].concat(_toConsumableArray(new Set(errors))).forEach(function (mes) {
	...
	function forEachPromise(fn) {
		return function (arr) {
			...
			return Promise.all(contents).then(function (res) {
				return [].concat(_toConsumableArray(new Set(res)));
			});
		};
	}
	...
	function getFileContents(err, filePaths) {
		...
		var flattenfilePaths = flatten(filePaths);

		var fn = function fn(resolve, file, index, arr) {
		...
		forEachPromise(fn)(flattenfilePaths).then(function (results) {
			processCSS(flattenfilePaths.map(function (i) {
				return results[0][i];
			}).join(''));
		}).catch(function (err) {
			return echo(err);
	...
	function flatten(arr) {
		return arr.reduce(function (a, b) {
			return [].concat(_toConsumableArray(a.concat(b)));
		}, []);
	}

	function run() {
		if (SOURCE) {
			Array.isArray(SOURCE) ? getFileContents(null, SOURCE) : getFileContents(null, [SOURCE]);
		} else {
			if (Array.isArray(DIR)) {
				var getFilePaths = function getFilePaths(resolve, file, index, arr) {
					(0, _glob2.default)(file + '/**/*.css', {}, function (err, paths) {
						return resolve(paths);
					});
				};

				forEachPromise(getFilePaths)(flatten(DIR)).then(function (results) {
					return getFileContents(null, results);
				});
			} else {
				(0, _glob2.default)(DIR + '/**/*.css', {}, getFileContents);
			}
		}
	}
